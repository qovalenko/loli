...
char *loli_logo = "LL             lll iii \n"
                  ...
                  "Loli programming language, v.0.5\n"
...
static void usage()
{
    ...
    fputs("Usage: loli [option] [file]\n"
          ...
          "-r             Render-mode\n"
          "-i string      Execute the inputed string\n"
...
int main(int argc, char **argv)
{
    ...
    if (is_file == 1)
      result = loli_load_file(state, to_process);
    else
      result = loli_load_string(state, "<cli>", to_process);

    if (result){
      if (do_tags == 0)
        result = loli_parse_content(state);
      else
        result = loli_render_content(state);
    }
    if (result == 0)
      fputs(loli_error_message(state), stderr);
